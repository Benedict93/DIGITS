...
def train(epoch, model, train_loader, optimizer):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        #compute output
        output = model(data)
        #define loss function and measure loss
        ...
        if batch_idx % log_interval == 0:
            ...
            logging.info("Training (epoch " + str(epoch) + "): " + "loss = " + str(losses.val) + 
                ", lr = " + str(args.lr_base_rate) + ", accuracy = {0:.2f}".format(accuracy.avg))
...
def main():
    ...
    if args.validation_db:
        validation_loader = torch.utils.data.DataLoader(validation_dataset, batch_size=args.batch_size, shuffle=args.shuffle, **kwargs)
